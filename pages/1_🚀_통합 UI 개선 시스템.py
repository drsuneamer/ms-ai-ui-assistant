...
# 회의록에서 요구사항 도출 > 코드 개선 > 통합 결과 전체를 한 화면에서 제공하는 페이지
...
- UX 라이팅 개선 (중우선순위)""",
                ...
                if st.button("📝 입력 완료", 
                            type="secondary", 
                            use_container_width=True,
                            key="direct_meeting_input_complete_btn"):
                    st.session_state["direct_meeting_input_ready"] = True
                    st.success("✅ 회의록 입력이 완료되었습니다!")
            
            # 분석 실행 버튼
            # 입력이 준비되었는지 확인
            is_meeting_ready = (
                (input_method == "📁 텍스트 파일 업로드" and meeting_content.strip()) or
                (input_method == "🎤 WAV 음성 파일 업로드" and meeting_content.strip()) or
                (input_method == "📝 직접 입력" and st.session_state.get("direct_meeting_input_ready", False))
            )
            
            if st.button("🔍 2단계: 요구사항 분석 실행", 
                        type="primary", 
                        use_container_width=True,
                        disabled=not is_meeting_ready,
                        key="requirements_analysis_btn"):
        ...
        with col2:
            ...
            if "requirements_analysis" in st.session_state:
                ...
                with st.expander("💡 입력 방식별 도움말"):
                    ...
                    **🎤 WAV 음성 파일 업로드:**
                    - Azure Speech Service 최적화
                    - 16-bit PCM, 16kHz, 모노 채널 권장
                    - 100MB 이하 제한
                    
                    **📝 직접 입력:**
                    - 회의 내용을 자세히 입력
                    - UI/UX 관련 피드백 구체적으로 작성
                    - 우선순위나 중요도 언급 권장
                    """)
    ...
    <form>
        ...
        <button class="login-btn">로그인</button>
                ...
                if st.button("📝 코드 입력 완료", 
                            ...
                            key="direct_code_input_complete_btn"):
                    st.session_state["direct_code_input_ready"] = True
            ...
            # 코드 입력이 준비되었는지 확인
            is_code_ready = (
                (code_input_method == "📁 파일 업로드" and current_code.strip()) or
                (code_input_method == "📝 직접 입력" and st.session_state.get("direct_code_input_ready", False))
            )
            
            if st.button("🚀 4단계: 코드 개선 실행", 
                        type="primary", 
                        use_container_width=True,
                        disabled=not is_code_ready,
                        key="code_improvement_btn"):
        ...
        with col2:
            ...
            if "improvement_result" in st.session_state:
                ...
                # 코드 입력 도움말
                with st.expander("💡 코드 입력 도움말"):
                    st.markdown("""
                    **📁 파일 업로드:**
                    - HTML, JS, JSX, JSP, VUE, TS, TXT 파일 지원
                    - UTF-8 인코딩 권장
                    - 자동 언어 감지 지원
